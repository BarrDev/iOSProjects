...
/*
 Firestore does not require you to manually create collections or documents.
 
 This extension creates an async wrapper for the setData(from:) method in the Cloud Firestore SDK, which accounts for some quirks in the original synchronous method. The original method throws for encoding errors, but passes all other errors to an optional completion handler. This means we’ll have no way of knowing if our document was saved unless we provide a completion handler. By contrast, our wrapper throws any errors that need to be brought to our attention—and if it doesn’t throw an error, we know our document was saved successfully.
 */
